...
function structureMovie(movies) {
    ...
    function createRating(movie, type) {
        ...
        if (movie.kp_vote > 100 && movie.imdb_vote > 100) {
            if (movie.kp_rating && movie.imdb_rating) {
                result.rating += Math.round(parseInt(movie.kp_rating)/2);
                result.rating += Math.round(parseInt(movie.imdb_rating)/2);
                result.vote += parseInt(movie.kp_vote);
                result.vote += parseInt(movie.imdb_vote);
            }
        }
        else if (movie.kp_vote > 100) {
            if (movie.kp_rating) {
                result.rating += Math.round(parseInt(movie.kp_rating)/2);
                result.vote += parseInt(movie.kp_vote);
            }
        }
        else if (movie.imdb_vote > 100) {
