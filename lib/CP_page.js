...
function pageIndex() {
    ...
    if (modules.schema.status) {
        page.schema = CP_schema.general(page);
    }
...
function pageMovie(type, movie, movies) {
    ...
    if (modules.schema.status) {
        page.schema = CP_schema.fullMovie(movie, movies);
    }
    ...
    function uniqueTitle() {
        return (texts.ids.indexOf(movie.kp_id)+1 && texts.movies[movie.kp_id].title)
            ? texts.movies[movie.kp_id].title
 ...
 * Adding basic information on the movie page:
 * protocol, domain, email, urls, codes, seo, title,
 * description, keywords, url, sorting, page.
 *
 * Data from modules:
 * social, schema.
 *
...
function pageCategory(query, sorting, num, movies) {
    ...
    for (var type in query) {
        if (query.hasOwnProperty(type)) {
            ...
            page.page        = {
                "current"     : num,
                "prev"        : pageUrl(page.url, sorting, num, movies.length, 'prev'),
                "next"        : pageUrl(page.url, sorting, num, movies.length, 'next')
            };
    ...
    if (modules.schema.status) {
        page.schema = CP_schema.category(page, movies);
    }
...
function pageCollection(url, sorting, num, movies) {
...
/**
 * Create a URL including num page.
 *
 * @param {String} url
 * @param {String} sorting
 * @param {Number} page
...
function pageUrl(url, sorting, page, count, type) {

    if (type == 'prev') {
        return (page - 1)
            ? url + '/' + (page - 1) + '?sorting=' + sorting
            : '';
    }
    else {
        return (count == config.default.count)
            ? url + '/' + (page + 1) + '?sorting=' + sorting
            : '';
    }
