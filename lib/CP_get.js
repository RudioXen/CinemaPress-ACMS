...
var sphinx = {
    "bests"  : 'bests_' + config.domain.replace(/[^A-Za-z0-9]/g,'_'),
...
/**
 * Gets an object with all categories of the site of a certain type (genre, actor, etc.).
 *
 * @param {String} category
 * @param {Callback} callback
 */

function categoriesGet(category, callback) {
    ...
    CP_sphinx.query(queryString, function (err, movies) {
        ...
        if (movies && movies.length) {
            categories = CP_structure.categories(movies);
        }
...
function moviesGet(query, count, sorting, page, callback) {
            ...
            if (query.hasOwnProperty(attribute)) {
                ...
                if (attribute == 'type') {
                    if (search == config.urls.types.serial) {
                        ...
                        where.push('`type` != 1');
                        match.push('@all_movies _all_ @genre !мультфильм !аниме !короткометражка !ток-шоу !новости !реальное !церемония !концерт !детский !документальный');
                    }
                    else {
